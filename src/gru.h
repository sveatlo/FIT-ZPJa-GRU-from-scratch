...
class GRU {
	public:
		GRU(map<char, unsigned> _char_to_idx, map<unsigned, char> _idx_to_char,  unsigned _vocab_size, unsigned _n_h = 100, unsigned _seq_len = 25, double _beta1 = 0.9, double _beta2 = 0.999);
	...
	private:
		...
		void clip_grads();
		void reset_grads();
		void update_params(double lr, int step);
		GRU_step_data forward_step(Matrix<double> x, Matrix<double> h_prev);
		GRU_backward_return backward_step(unsigned target_idx, Matrix<double> dh_next, Matrix<double> r, Matrix<double> z, Matrix<double> h_hat, Matrix<double> h, Matrix<double> p, Matrix<double> x, Matrix<double> h_prev);
